...
AddEventHandler('gameEventTriggered', function(event, args)
	if event == 'CEventNetworkEntityDamage' then
		...
		if targetPed == cache.ped then
			...
			local hit = GetPedLastDamageBone(cache.ped)

			if hit then
				SendNUIMessage({ action = 'showInjury', data = true })

				-- After 5 minutes hide the injury in inv
				SetTimeout(60000 * 5, function()
					SendNUIMessage({ action = 'showInjury', data = false })
				end)
			end
		end
	end
