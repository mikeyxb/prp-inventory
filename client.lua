...
CreateThread(function()
	while true do
		...
		if not plyHealth then plyHealth = health end
		if not plyArmour then plyArmour = armour end
		
		local armourDamaged = plyArmour ~= armour and armour < plyArmour and armour > 0
		local healthDamaged = plyHealth ~= health and health < plyHealth and health > 0

		if armourDamaged or healthDamaged then
			local hit, boneIndex = GetPedLastDamageBone(playerPed)
			
			if hit and parts[boneIndex] ~= 'NONE' then
